...
public final class GlobalSetupUtil {
    ...
    public static void run(Logger log, Properties props, Map<String, String> vars) {
        ...
        if (verboseLogging) {
            log.info("GlobalSetupUtil starting with features: template={}, directory={}, hostname={}, environment={}, git={}", 
                    templateProcessing, directorySetup, hostnameDetection, environmentDetection, gitDetection);
        }
    ...
    private static void processTemplateProperties(Logger log, Properties props, Map<String, String> vars) {
        ...
        log.info("Template property processing complete ({} entries)", count);
    }
    ...
    private static void checkHost(Logger log, Properties props, Map<String, String> vars) {
        ...
        boolean remote = host.startsWith("jm-gen") || host.startsWith("jm-nofrills");
        if (remote) {
            String remoteHost = props.getProperty("V_STS_LOCAL_HOST", "");
            if (!remoteHost.isBlank()) {
                props.setProperty("V_STS_HOST", remoteHost);
                vars.put("V_STS_HOST", remoteHost);
                log.info("Remote run → V_STS_HOST set to {}", remoteHost);
            }
        } else {
            String def = props.getProperty("V_STS_HOST_DEFAULT", "");
            if (!def.isBlank()) {
                props.setProperty("V_STS_HOST", def);
                vars.put("V_STS_HOST", def);
                log.info("Local run → V_STS_HOST set to {}", def);
            }
        }
        props.setProperty("IS_LOCAL_ENVIRONMENT", String.valueOf(!remote));
    }
        ...
        while (ptr != null) {
            ...
            if (Files.exists(cfg)) {
                try {
                    ...
                    log.info("Git repository detected: {} (url: {})", name, url);
